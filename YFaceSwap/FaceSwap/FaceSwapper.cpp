...
void FaceSwapper::getFacePoints(const cv::Mat &frame)
{
	...
	cv::rectangle(frame, rect_ann.tl(), rect_ann.br(), (55, 255, 155), 5);
	cv::rectangle(frame, rect_bob.tl(), rect_bob.br(), (55, 255, 155), 5);
	for (int i = 0; i < 68; i++)
	{
		cv::circle(frame, getPoint(0, i), 2, cv::Scalar(0, 255, 0));
		cv::circle(frame, getPoint(1, i), 2, cv::Scalar(0, 255, 0));
	}
	cv::imshow("face keypoint frame", frame);
	cv::waitKey();
...
cv::Mat FaceSwapper::getRefinedMasks()
{
	...
	cv::imshow("refined_masks", refined_masks);
	cv::waitKey();
...
cv::Mat FaceSwapper::getWarppedFaces()
{
	...
	//cv::imshow("warpped_faces", warpped_faces);
	//cv::waitKey();
...
void FaceSwapper::colorCorrectFaces()
{
	...
	//cv::imshow("small_frame(big_rect_ann)", small_frame(big_rect_ann));
	//cv::imshow("warpped_faces(big_rect_ann)", warpped_faces(big_rect_ann));
	//cv::imshow("warpped_mask_bob(big_rect_ann)", warpped_mask_bob(big_rect_ann));	
	//cv::waitKey();


	//cv::imshow("small_frame(big_rect_bob)", small_frame(big_rect_bob));
	//cv::imshow("warpped_faces(big_rect_bob)", warpped_faces(big_rect_bob));
	//cv::imshow("warpped_mask_ann(big_rect_bob)", warpped_mask_ann(big_rect_bob));
	cv::waitKey();
